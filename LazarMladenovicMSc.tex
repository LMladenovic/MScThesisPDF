...
\item  \texttt{\texttt{List} \textit{re.findall}(\texttt{string} pattern, \texttt{string} string, \texttt{int} flags=0)} - Vraća sva podudaranja \texttt{pattern}-a u \texttt{string}-u kao listu niski.
...
\item \texttt{changedLine} - sadrži tačnu liniju koda u datoteci \textit{changedFile} koja je promenjena.
